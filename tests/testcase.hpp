 ...
 */
struct Testcase {
	std::string alphabet; ///< Characters of the alphabet.
	CostModel cost{1, 1}; ///< Parsed edit operation costs.
...
inline Testcase parse_testcase(const fs::path &file) {
	...
	// 2. Cost model: ins del match mismatch.
	{
		std::istringstream iss(next("cost model"));
		int ins = 0, del = 0, match = 0, mismatch = 0;
		if (!(iss >> ins >> del >> match >> mismatch))
			fail(file, line_no, "expected 'ins del match mismatch'");
		tc.cost = CostModel(ins, del, match, mismatch);
	}
